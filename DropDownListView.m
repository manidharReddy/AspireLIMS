...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        if (isMultipleSelection) {
            ...
            //[btnDone setFrame:CGRectMake(rect.origin.x+10,10, 82, 31)];
            [btnDone setFrame:CGRectMake(_kTableView.frame.size.width-100,10, 82, 31)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.arryData containsObject:indexPath]){
        imgarrow.frame=CGRectMake(_kTableView.frame.size.width-27,2, 27, 27);
